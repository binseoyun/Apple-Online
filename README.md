...
**솔로 연습 모드**도 지원합니다. 상대 없이 혼자 AI 봇과 대결하여 실력을 키울 수 있습니다. Easy / Normal / Hard 3가지 난이도를 선택할 수 있으며, 솔로 결과는 랭킹에 반영되지 않습니다.
...
- **🤖 솔로 연습** 버튼을 통해 AI 봇과 단독 대결 (난이도 선택 모달)
- 사과 게임 **튜토리얼** 확인
...
- 로비에서 **솔로 연습** 버튼 클릭 → **난이도 선택 모달** 등장
- Easy / Normal / Hard 중 선택하면 전용 솔로 방이 즉시 생성됨
- AI 봇(`🤖 Bot`)을 상대로 1분간 대결
- 봇은 서버 사이드 로직으로 동작 (브라우저 없음, WebRTC 없음)
  - **Easy**: 4~7초 간격, 랜덤 사각형 선택
  - **Normal**: 2~4초 간격, 랜덤 사각형 선택
  - **Hard**: 0.6~1.8초 간격, 점수 최대 사각형 우선 선택
- 솔로 게임 결과는 **랭킹(ELO) 에 반영되지 않음**
- 상대방 WebRTC 커서 공유 기능 생략(봇이므로 불필요)
...
- 혼자 연습할 수 있도록 AI 봇을 서버 사이드에서 구현했다.
- 봇은 브라우저 없이 `botHandler.js`의 순수 로직으로 동작하며, Redis에서 맵 데이터를 직접 읽고 수정한다.
- `findAllValidMoves()` 함수로 합이 10이 되는 모든 사각형을 탐색(O(n⁴) brute-force)하고 난이도에 따라 행동한다.
- 솔로 방은 Redis에만 저장되고, 게임 종료 시 DB/ELO 처리를 건너뛴다.
